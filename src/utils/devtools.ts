...
export async function main(ns: NS) {
  ...
  let stateCalls = 0;
  let resolve: any;
  const nextLevelHook = (callNumber: number, fn: any, parentThis: any, parentArgs: any) => {
  ...
  const resultP = Promise.race([
    ...
    ns.asleep(5000).then(() => {
      throw Error('Something unknown went wrong while running exploit');
    }),
  ]).finally(() => {
    React.createElement = orig;
    React.useState = origState;
  ...
  const [state, setState] = (await resultP) as any;
