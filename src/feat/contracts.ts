...
function solveLZCompress(s: string): string {
	// dp[pos][type] = shortest encoded string to compress s[0..pos-1] with next chunk type
	const n = s.length;
	...
	for (let pos = 0; pos <= n; pos++) {
		for (let type = 1; type <= 2; type++) {
			...
			if (type === 1) {
				// emit type-1 literal chunk of length l
				...
				// emit type-2 back-reference chunk
				// length 0: no reference byte, skip type-2
				...
				for (let l = 1; l <= 9 && pos + l <= n; l++) {
					for (let ref = 1; ref <= 9 && ref <= pos; ref++) {
						// check if s[pos..pos+l-1] matches back-reference at distance ref
						let match = true;
						for (let j = 0; j < l; j++) {
